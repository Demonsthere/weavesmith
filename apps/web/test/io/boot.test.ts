...
describe('bootPattern', () => {
  ...
  it('falls back to the default band on a cold start', () => {
    expect(bootPattern('')).toEqual({ pattern: defaultPattern(), problems: null });
  });
  ...
  it('prefers the share link over the autosave', () => {
    ...
    autosave(named('Mine'));
    expect(bootPattern(`#p=${encodePattern(named('Theirs'))}`).pattern.meta.name).toBe(
      'Theirs',
    );
  });
  ...
  it('ignores a hash that is not a share link', () => {
    expect(bootPattern('#/chart')).toEqual({ pattern: defaultPattern(), problems: null });
  });
});
